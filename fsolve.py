   ...
   # f_liq = 0
    ...
    for p in range(N_iter):
        ...
        for j in range(1,M-1):
            ...
            k = j*N
            rr = d[k]*FF[0,j] + l1[k]*FF[1,j] + l2[k]*FF[0,j+1] - f[0,j]          
            ...
            for i in range(1,N-1):
                k = i+j*N
                rr = l2_[k]*FF[i,j-1] + l1_[k]*FF[i-1,j] + d[k]*FF[i,j] + l1[k]*FF[i+1,j] + l2[k]*FF[i,j+1] - f[i,j]
            ...
            # справа
            k = (j+1)*N-1
            rr =  rr = l2_[k]*FF[N-1,j-1] + l1_[k]*FF[N-2,j] + d[k]*FF[N-1,j]  - f[N-1,j]
